...
use crate::gap2_tail_emit_v::{lemma_surge_emit_return_block1_tail_safe_v, lemma_surge_emit_return_block3_tail_safe_v};
verus! {
...
// ─────────────────────────────────────────────────────────────────────────────
// v-side (α-tail) companions — mirror of the u-side seret phase tail_safe lemmas.
// Surge moves R toward the v-tail, so each needs the surge margin on h.
// ─────────────────────────────────────────────────────────────────────────────

pub proof fn lemma_seret1x_phase_tail_safe_v(tm: Tm, len: nat, pc: nat, big_u: nat, od: Seq<nat>, s: nat,
    qexit: nat, jl1: int, jl2: int, jl3: int, jl4: int, h: nat)
    requires
        tm_wf(tm),
        tm.n == 5,
        ...
        h >= od.len() + 2,
    ensures
        tail_safe_v(tm, TmConfig { u: big_u, v: dpack(od, tm.m), a: 0, q: entry5(pc) },
            (2 * od.len() + 4) as nat, h),
        tail_end_h_v(tm, TmConfig { u: big_u, v: dpack(od, tm.m), a: 0, q: entry5(pc) },
            (2 * od.len() + 4) as nat, h) == h,
{
    assert(pc * 288 + 288 <= 288 * (len + 1)) by(nonlinear_arith) requires pc <= len;
    let base = (pc * 288) as int;
    ...
    // ── invoke the verified singleton step with q_home = qexit, walk-back quints jl1..jl4. ──
    lemma_surge_emit_return_block1_tail_safe_v(tm, big_u, od, s,
        q_iter, q_surge, q_eret, qexit,
        i_pivot_r, ir1, ir2, ir3, ir4,
        i_emit, i_off_l, jl1, jl2, jl3, jl4, h);
}

pub proof fn lemma_seret3x_phase_tail_safe_v(tm: Tm, len: nat, pc: nat, big_u: nat, od: Seq<nat>,
    s0: nat, s1: nat, s2: nat, qexit: nat, jl1: int, jl2: int, jl3: int, jl4: int, h: nat)
    requires
        tm_wf(tm),
        tm.n == 5,
            ...
            (2 * od.len() + 8) as nat, h) == h,
{
    ...
    // ── invoke the verified triple-singleton step with q_home = qexit, walk-back quints jl1..jl4. ──
    lemma_surge_emit_return_block3_tail_safe_v(tm, big_u, od, s0, s1, s2,
        q_iter, q_surge, q_e1, q_e2, q_eret, qexit,
        i_pivot_r, ir1, ir2, ir3, ir4,
        i_e0, i_e1, i_e2, i_off_l, jl1, jl2, jl3, jl4, h);
}
