//! # GAP-2 G2-F — RIGHT-tail (α-block) safety of the per-power-block periodic step (`v`-side mirror)
//!
//! The `v`-side analog of [`crate::gap2_tail_power`]. Each power-block step is `copy_refresh ∘ block_loop`;
//! this discharges its [`tail_safe_v`] by the **2-piece chain** — [`lemma_copy_refresh_tail_safe_v`]
//! (unconditional) ∘ [`lemma_block_loop_block1_tail_safe_v`] (the surge constraint) — at the constant
//! α-offset `h`, both net-displacement-0. The binding constraint comes from the consume loop's surge over
//! the growing output: `h ≥ od.len() + M + 1` (block1), `h ≥ od.len() + 3·M + 1` (block3); the `M = 1`
//! variants need `od.len() + 2` / `od.len() + 4`. Parametric in `h`. Fully verified, no escape hatches.
...
use crate::tm_copy_refresh::{copy_u, copy_refresh_fuel, lemma_copy_refresh, lemma_copy_refresh_m1};
...
use crate::gap2_tail_lift_v::{tail_safe_v, tail_end_h_v, lemma_tail_v_chain};
use crate::gap2_tail_phase1_v::lemma_copy_refresh_tail_safe_v;
use crate::gap2_tail_phase1_m1_v::lemma_copy_refresh_m1_tail_safe_v;
use crate::gap2_tail_emit_v::{lemma_block_loop_block1_tail_safe_v, lemma_block_loop_block3_tail_safe_v};

verus! {

/// **`power_block_step_block1` is `v`-tail-safe** for its `power_block_fuel_b1` steps, net-disp-0 when
/// `h ≥ od.len() + M + 1`. `v`-side mirror of
/// [`crate::gap2_tail_power::lemma_power_block_step_block1_tail_safe`].
pub proof fn lemma_power_block_step_block1_tail_safe_v(
    ...
    i_pivot: int, i_one_l: int, i_erase: int, i_disc: int, h: nat,
)
    requires
        tm_wf(tm),
        tm.n >= 5,
    ...
    ensures
        ...
        tail_end_h_v(tm,
            TmConfig { u: copy_u(0, big_m, g, tm.m), v: dpack(od, tm.m), a: 0, q: q_dh0 },
            power_block_fuel_b1(big_m, g, od.len()), h) == h,
{
    let m = tm.m;
    reveal(tm_wf);
    let c0 = TmConfig { u: copy_u(0, big_m, g, m), v: dpack(od, m), a: 0, q: q_dh0 };

    // ── PHASE A — COPY-REFRESH bridge: tm_run(c0, crf) == c1, tail_safe_v at h → h. ──
    ...
    lemma_copy_refresh_tail_safe_v(tm, big_m, g, dpack(od, m),
        q_dh0, q_dw0, q_bk0, q_t0, q_a0, q_rf0, q_rg0,
        q_home, q_t, q_a, q_b, q_rf, q_rg, q_rt, q_dw,
        q_turn, q_turng, q_ret,
        q_ut, q_ua, q_uf, q_ur, q_urg, q_urt,
    ...
    // ── PHASE B — CONSUME LOOP: tail_safe_v(c1, loop_fuel, h) → h (needs h ≥ |od|+M+1). ──
    lemma_block_loop_block1_tail_safe_v(tm, big_m, w, od, s,
        ...
        i_pivot, i_one_l, i_erase, i_disc, i_urtemp, h);

    // ── chain: COPY-REFRESH ∘ CONSUME-LOOP at h (both net-disp-0). ──
    lemma_tail_v_chain(tm, c0, copy_refresh_fuel(big_m, g), loop_fuel_b1(od.len(), big_m), h, h, h);
    assert(power_block_fuel_b1(big_m, g, od.len())
        == (copy_refresh_fuel(big_m, g) + loop_fuel_b1(od.len(), big_m)) as nat);
}

/// **`power_block_step_block3` is `v`-tail-safe**, net-disp-0 when `h ≥ od.len() + 3·M + 1`. `v`-side mirror
/// of [`crate::gap2_tail_power::lemma_power_block_step_block3_tail_safe`].
pub proof fn lemma_power_block_step_block3_tail_safe_v(
    ...
    ensures
        tail_safe_v(tm,
            TmConfig { u: copy_u(0, big_m, g, tm.m), v: dpack(od, tm.m), a: 0, q: q_dh0 },
            power_block_fuel_b3(big_m, g, od.len()), h),
        tail_end_h_v(tm,
            TmConfig { u: copy_u(0, big_m, g, tm.m), v: dpack(od, tm.m), a: 0, q: q_dh0 },
            power_block_fuel_b3(big_m, g, od.len()), h) == h,
{
    let m = tm.m;
    reveal(tm_wf);
    let c0 = TmConfig { u: copy_u(0, big_m, g, m), v: dpack(od, m), a: 0, q: q_dh0 };
    ...
    lemma_block_loop_block3_tail_safe_v(tm, big_m, w, od, s0, s1, s2,
        q_urt, q_guard, q_iter, q_surge, q_e1, q_e2, q_eret, q_bhome, q_dwalk, q_disc, q_exit,
        i_peek, i_cont, i_exit, i_pivot_r, ir1, ir2, ir3, ir4,
        i_e0, i_e1, i_e2, i_off_l, il1, il2, il3, il4,
        i_pivot, i_one_l, i_erase, i_disc, i_urtemp, h);

    lemma_tail_v_chain(tm, c0, copy_refresh_fuel(big_m, g), loop_fuel_b3(od.len(), big_m), h, h, h);
    assert(power_block_fuel_b3(big_m, g, od.len())
        == (copy_refresh_fuel(big_m, g) + loop_fuel_b3(od.len(), big_m)) as nat);
}

/// **`power_block_step_block1_m1` is `v`-tail-safe**, net-disp-0 when `h ≥ od.len() + 2`. `v`-side mirror of
/// [`crate::gap2_tail_power::lemma_power_block_step_block1_m1_tail_safe`].
pub proof fn lemma_power_block_step_block1_m1_tail_safe_v(
    ...
    ensures
        ...
        tail_end_h_v(tm,
            TmConfig { u: copy_u(0, 1, g, tm.m), v: dpack(od, tm.m), a: 0, q: q_dh0 },
            power_block_fuel_b1_m1(g, od.len()), h) == h,
{
    let m = tm.m;
    reveal(tm_wf);
    ...
    lemma_copy_refresh_m1_tail_safe_v(tm, g, dpack(od, m),
        q_dh0, q_dw0, q_bk0, q_t0, q_a0, q_rf0, q_rg0,
        q_t, q_a, q_b, q_turn, q_turng, q_ret, q_home,
        q_ut, q_ua, q_uf, q_ur, q_urg, q_urt,
        i_dpeel, i_dtemp, i_dins, i_dwb,
    ...
    lemma_block_loop_block1_tail_safe_v(tm, 1, w, od, s,
        q_urt, q_guard, q_iter, q_surge, q_eret, q_bhome, q_dwalk, q_disc, q_exit,
        i_peek, i_cont, i_exit, i_pivot_r, ir1, ir2, ir3, ir4,
        i_emit, i_off_l, il1, il2, il3, il4,
        i_pivot, i_one_l, i_erase, i_disc, i_one_r, h);

    lemma_tail_v_chain(tm, c0, (6 * g + 12) as nat, loop_fuel_b1(od.len(), 1), h, h, h);
    assert(power_block_fuel_b1_m1(g, od.len()) == ((6 * g + 12) + loop_fuel_b1(od.len(), 1)) as nat);
}

/// **`power_block_step_block3_m1` is `v`-tail-safe**, net-disp-0 when `h ≥ od.len() + 4`. `v`-side mirror of
/// [`crate::gap2_tail_power::lemma_power_block_step_block3_m1_tail_safe`].
pub proof fn lemma_power_block_step_block3_m1_tail_safe_v(
    ...
    ensures
        tail_safe_v(tm,
            TmConfig { u: copy_u(0, 1, g, tm.m), v: dpack(od, tm.m), a: 0, q: q_dh0 },
            power_block_fuel_b3_m1(g, od.len()), h),
        tail_end_h_v(tm,
            TmConfig { u: copy_u(0, 1, g, tm.m), v: dpack(od, tm.m), a: 0, q: q_dh0 },
            power_block_fuel_b3_m1(g, od.len()), h) == h,
{
    let m = tm.m;
    reveal(tm_wf);
    let c0 = TmConfig { u: copy_u(0, 1, g, m), v: dpack(od, m), a: 0, q: q_dh0 };
    ...
    lemma_block_loop_block3_tail_safe_v(tm, 1, w, od, s0, s1, s2,
        q_urt, q_guard, q_iter, q_surge, q_e1, q_e2, q_eret, q_bhome, q_dwalk, q_disc, q_exit,
        i_peek, i_cont, i_exit, i_pivot_r, ir1, ir2, ir3, ir4,
        i_e0, i_e1, i_e2, i_off_l, il1, il2, il3, il4,
        i_pivot, i_one_l, i_erase, i_disc, i_one_r, h);

    lemma_tail_v_chain(tm, c0, (6 * g + 12) as nat, loop_fuel_b3(od.len(), 1), h, h, h);
